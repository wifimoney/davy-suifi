...
module davy::intent_tests {
    ...
    fun create_range_price_offer(scenario: &mut ts::Scenario, clock: &Clock) {
        ...
        offer::create<TOKEN_A, USDC>(
            coin,
            1_000_000_000, // min_price: 1 USDC per TOKEN_A
            3_000_000_000, // max_price: 3 USDC per TOKEN_A
            10_000,
            1,
            100,
            clock,
    ...
    fun test_create_intent() {
        ...
        let clock = setup_clock(&mut scenario);
        {
            ...
            intent::create_price_bounded<TOKEN_A, USDC>(
                ...
                3_000_000_000, // max_price
                10_000,        // expiry
                &clock,
                scenario.ctx(),
            );
        };
        // Verify intent exists as shared object
    ...
    // ===== 2. Execute against offer — happy path (full fill) =====
    ...
    fun test_execute_against_offer_full() {
        ...
        // Maker creates offer: 1000 TOKEN_A, fixed price 2e9
        scenario.next_tx(MAKER);
        create_standard_offer(&mut scenario, &clock);

        // Taker creates intent: wants 1000 TOKEN_A, pays up to 5000 USDC
        scenario.next_tx(TAKER);
        {
            let payment = coin::mint_for_testing<USDC>(5000, scenario.ctx());
            intent::create_price_bounded<TOKEN_A, USDC>(
        ...
        // Executor matches intent against offer
        scenario.next_tx(EXECUTOR);
        {
            ...
            // Intent should be executed
            assert!(intent::intent_status(&intent) == intent::status_executed());
            // Escrow should be drained (paid 2000, refunded 3000)
            assert!(intent::escrowed_amount(&intent) == 0);
            // Offer should be fully filled
        ...
        // Verify asset routing: TAKER gets TOKEN_A, MAKER gets USDC
        scenario.next_tx(TAKER);
        {
            // Taker should have 1000 TOKEN_A
            ...
            // Taker should have USDC refund (5000 - 2000 = 3000)
        ...
        scenario.next_tx(MAKER);
        {
            // Maker should have 2000 USDC payment
    ...
    fun test_execute_partial_fill() {
        ...
        // Maker: 1000 TOKEN_A, fixed price 2e9, partial allowed
        scenario.next_tx(MAKER);
        create_standard_offer(&mut scenario, &clock);
        ...
        scenario.next_tx(EXECUTOR);
        {
            ...
            assert!(intent::intent_status(&intent) == intent::status_executed());
            // Offer partially filled: 500 remaining
        ...
        // Verify: taker gets 500 TOKEN_A + 2000 USDC refund (paid 1000)
        scenario.next_tx(TAKER);
        {
            let token_coin = scenario.take_from_sender<Coin<TOKEN_A>>();
    ...
    fun test_cancel_not_creator_aborts() {
        ...
        {
            let mut intent = scenario.take_shared<ExecutionIntent<TOKEN_A, USDC>>();
            intent::cancel(&mut intent, scenario.ctx()); // not creator → abort 206
            ts::return_shared(intent);
        };
        clock::destroy_for_testing(clock);
        scenario.end();
    ...
    fun test_expire_intent() {
        ...
        // Refund goes to original creator (TAKER), not caller
        scenario.next_tx(TAKER);
        {
            let refund = scenario.take_from_sender<Coin<USDC>>();
            assert!(coin::value(&refund) == 5000);
    ...
    fun test_expire_before_time_aborts() {
        ...
        // Clock still at 1000 < 10_000 expiry
        scenario.next_tx(NOBODY);
        {
            let mut intent = scenario.take_shared<ExecutionIntent<TOKEN_A, USDC>>();
            intent::expire_intent(&mut intent, &clock, scenario.ctx()); // abort 201
            ts::return_shared(intent);
        };
        clock::destroy_for_testing(clock);
        scenario.end();
    ...
    // ===== 9. Execute already-executed intent aborts (200) =====
    ...
    fun test_double_execute_aborts() {
        ...
        // Create second offer for the retry
        scenario.next_tx(MAKER);
        create_standard_offer(&mut scenario, &clock);

        // Second execution aborts — intent already executed
        scenario.next_tx(EXECUTOR);
        {
            let mut intent = scenario.take_shared<ExecutionIntent<TOKEN_A, USDC>>();
            let mut offer = scenario.take_shared<LiquidityOffer<TOKEN_A, USDC>>();
    ...
    fun test_insufficient_offer_liquidity_aborts() {
        ...
        // Taker: wants 1000 TOKEN_A (more than offer has)
        scenario.next_tx(TAKER);
        {
            let payment = coin::mint_for_testing<USDC>(5000, scenario.ctx());
            intent::create_price_bounded<TOKEN_A, USDC>(
    ...
    fun test_insufficient_escrow_aborts() {
        ...
        // Maker: 1000 TOKEN_A at price 2e9 → requires 2000 USDC
        scenario.next_tx(MAKER);
        create_standard_offer(&mut scenario, &clock);

        // Taker: only escrows 500 USDC (not enough for 1000 @ 2 USDC/TOKEN_A)
        scenario.next_tx(TAKER);
        {
    ...
    // ===== 12. Price mismatch — offer price above intent max =====
    ...
    fun test_price_mismatch_aborts() {
        ...
        // Maker: 1000 TOKEN_A at min_price 5e9 (5 USDC per TOKEN_A)
        scenario.next_tx(MAKER);
        {
            let coin = coin::mint_for_testing<TOKEN_A>(1000, scenario.ctx());
            offer::create<TOKEN_A, USDC>(
                coin,
                5_000_000_000, // min_price: 5 USDC/TOKEN_A
                5_000_000_000,
                10_000, 1, 100,
                &clock, scenario.ctx(),
            );
        ...
        // Taker: max_price only 3e9 (won't pay more than 3 USDC/TOKEN_A)
        scenario.next_tx(TAKER);
        {
            ...
            intent::create_price_bounded<TOKEN_A, USDC>(
                1000, payment,
                1_000_000_000,
                3_000_000_000, // max 3 USDC — offer wants 5 → mismatch
                10_000, &clock, scenario.ctx(),
            );
        };
    ...
    // ===== 13. Create with zero receive_amount aborts (208) =====
    ...
    fun test_create_zero_receive_aborts() {
        ...
        let clock = setup_clock(&mut scenario);
        {
            let payment = coin::mint_for_testing<USDC>(5000, scenario.ctx());
            intent::create_price_bounded<TOKEN_A, USDC>(
                0, payment, // zero receive → abort 208
                1_000_000_000, 3_000_000_000,
                10_000, &clock, scenario.ctx(),
            );
        };
    ...
    // ===== 14. Create with zero payment aborts (209) =====
    ...
    fun test_create_zero_payment_aborts() {
        ...
        let clock = setup_clock(&mut scenario);
        {
            ...
            intent::create_price_bounded<TOKEN_A, USDC>(
                1000, payment, // zero payment → abort 209
                1_000_000_000, 3_000_000_000,
                10_000, &clock, scenario.ctx(),
            );
        };
    ...
    // ===== 15. Create with expired timestamp aborts (207) =====
    ...
    fun test_create_expired_aborts() {
        ...
        let clock = setup_clock(&mut scenario); // clock at 1000
        {
            ...
            intent::create_price_bounded<TOKEN_A, USDC>(
                1000, payment,
                1_000_000_000, 3_000_000_000,
                500, // expiry 500 < clock 1000 → abort 207
                &clock, scenario.ctx(),
            );
        };
        clock::destroy_for_testing(clock);
    ...
    // ===== 16. Cancel already-cancelled intent aborts (200) =====
    ...
    fun test_double_cancel_aborts() {
        ...
        {
            let mut intent = scenario.take_shared<ExecutionIntent<TOKEN_A, USDC>>();
            intent::cancel(&mut intent, scenario.ctx()); // already cancelled → abort 200
            ts::return_shared(intent);
        };
        clock::destroy_for_testing(clock);
        scenario.end();
    ...
    fun test_execute_uses_offer_min_price() {
        ...
        // At min_price 1e9: payment = 1000 * 1e9 / 1e9 = 1000 USDC
        // Refund = 5000 - 1000 = 4000
        scenario.next_tx(MAKER);
        {
            let pay_coin = scenario.take_from_sender<Coin<USDC>>();
            assert!(coin::value(&pay_coin) == 1000); // min price
            scenario.return_to_sender(pay_coin);
        };
